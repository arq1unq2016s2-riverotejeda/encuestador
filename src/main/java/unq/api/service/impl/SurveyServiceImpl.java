...
public class SurveyServiceImpl implements SurveyService {
	...
	public List<ClassOccupation> getClassOccupation() {
		...
		for (Survey survey : surveys) {
			// get chosen subjects
			Stream<SelectedSubject> selectedSubjects = survey.getSelectedSubjects().stream().filter(this::isSelected);
			totalSelectedSubjects.addAll(selectedSubjects.collect(Collectors.toList()));
		}

		Map<String, List<SelectedSubject>> seleccionBySubject = totalSelectedSubjects.stream()
				.collect(Collectors.groupingBy(SelectedSubject::getSubject));

		seleccionBySubject.forEach((subjectName, value) -> {
			Map<String, Long> countComision = value.stream()
					.collect(Collectors.groupingBy(SelectedSubject::getStatus, Collectors.counting()));
			...
			countComision.forEach((comision, count) -> {
				...
				Stream<Division> divisions = subjectStream.collect(Collectors.toList()).get(0).getDivisions().stream()
						.filter(division -> division.getComision().name().equals(comision.split(":")[0]));
	...
	public double getPercentageCompletedSurveys() {
		...
		for (Student student : students) {
			if (this.completedSurvey(student.getLegajo(), surveys))
				counter++;
		}
	...
	private List<String> buildSelectionDates(List<Division> divisions) {
		return divisions.stream().map(subjectElection -> subjectElection.getComision() + ": "
				+ String.join(", ", subjectElection.getWeekdays())).collect(Collectors.toList());
	}
